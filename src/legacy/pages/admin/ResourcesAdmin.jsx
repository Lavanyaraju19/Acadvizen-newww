...
export function ResourcesAdmin() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const { error } = await supabase.from('resources').upsert(rowPayload, { onConflict: 'id' }).select()
      ...
      await fetch('/api/revalidate', {
        ...
        body: JSON.stringify({ paths: ['/', '/courses', '/tools'] }),
      })
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          <Surface className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {errorMessage && <div className="text-sm text-rose-300">{errorMessage}</div>}
              <div>
      ...
      {loading ? (
        ...
        <Surface className="p-6 text-sm text-slate-400">No content yet. Click New to add your first entry.</Surface>
      ) : (
