...
const trainerBackdrop = techGridPanelStyle
const shimmerDustPanelStyle = neonBlueprintPanelStyle
      ...
      <Section className="py-10 md:py-12">
        <Container className="max-w-6xl">
          <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr] items-center">
            <Surface className="p-5 bg-transparent backdrop-blur-none border-[#1b3551]" style={techGridPanelStyle}>
              <AdaptiveImage
            ...
            <Surface className="p-7 md:p-9 bg-transparent backdrop-blur-none border-[#1b3551]" style={neonBlueprintPanelStyle}>
      ...
      <Section className="py-10 md:py-12">
        <Container className="max-w-6xl">
          <div className="grid gap-6 lg:grid-cols-2">
            <div className={`${solidPublicPanelClass} p-7 md:p-9`} style={shimmerDustPanelStyle}>
            ...
            <div className={`${solidPublicPanelClass} p-7 md:p-9`} style={techGridPanelStyle}>
          ...
          <div className={`${solidPublicPanelClass} mt-6 p-7 md:p-9`} style={shimmerDustPanelStyle}>
          ...
          <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-5">
            {aboutWhyChoose.map((item, index) => (
              <Link key={item.title} to={item.to} className="block">
                <div
                  className={`${solidPublicPanelClass} h-full p-6 transition-transform duration-200 hover:-translate-y-1`}
                  style={index % 2 === 0 ? techGridPanelStyle : neonBlueprintPanelStyle}
                >
          ...
          >
            ...
            >
              {aboutTrainers.map((trainer, index) => (
                <article
                  key={trainer.name}
                  className="flex min-h-[31rem] min-w-[255px] snap-start flex-col overflow-hidden rounded-[1.8rem] border-[3px] border-black/70 bg-white shadow-[0_22px_38px_rgba(0,0,0,0.24)] sm:min-w-[285px] lg:min-w-[300px]"
                >
                  <div style={{ backgroundColor: trainerPalette[index % trainerPalette.length] }}>
                    <ConsistentPortraitMedia
                      src={trainer.image}
                      alt={trainer.name}
                      className={trainer.name === 'Dinesh Karthik' ? 'h-[340px]' : ''}
                      scale={trainer.imageScale ?? 1.12}
                    />
                  </div>
                  <div className="bg-white px-4 pb-5 pt-4 text-slate-950 sm:px-5">
                    <h3 className="text-[1.15rem] font-bold leading-tight tracking-tight text-slate-950 sm:text-[1.35rem]">{trainer.name}</h3>
                    <p className="mt-2 text-[0.98rem] leading-6 text-slate-900/85">{trainer.designation}</p>
                  </div>
                </article>
              ))}
            </div>
