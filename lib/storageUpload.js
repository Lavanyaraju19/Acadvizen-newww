...
export async function replaceFileAsset(mediaId, file) {
  if (!mediaId) {
  ...
  const payload = await adminApiFetch(`/api/cms/media/${mediaId}/replace`, {
    method: 'POST',
    body: formData,
  })

  if (!payload?.data?.url) {
    throw new Error('Replace completed without a public URL.')
  }
