...
export const BLOG_SLUG_ALIASES = {
  ...
  'how-to-crack-the-google-and-accenture-digital-marketing-interview-tips': 'how-to-crack-the-google-and-accenture-digital-marketing-interview',
...
// `data/blogs.js` is a large hand-maintained array literal; a stray comma there creates a
// sparse-array hole (an elided element), and `Array.prototype.find`/`forEach` visit holes and
// invoke the callback with `undefined`, which crashes any `.slug`/`.id` access on the result.
// Every direct consumer of the raw `blogs` export should go through this instead so one bad
// entry can never take down a whole page.
export function getSafeLocalBlogs(rawBlogs = localBlogs) {
  if (!Array.isArray(rawBlogs)) return []
  const seenSlugs = new Set()
  const safe = []
  for (const entry of rawBlogs) {
    const isValidEntry = entry && typeof entry === 'object' && typeof entry.slug === 'string' && entry.slug.trim()
    if (!isValidEntry) {
      if (process.env.NODE_ENV !== 'production') {
        console.warn('[blogSlugResolver] Skipping malformed local blog entry.', entry)
      }
      continue
    }
    if (seenSlugs.has(entry.slug)) {
      if (process.env.NODE_ENV !== 'production') {
        console.warn('[blogSlugResolver] Skipping duplicate local blog slug.', entry.slug)
      }
      continue
    }
...
export const SAFE_LOCAL_BLOGS = getSafeLocalBlogs()
