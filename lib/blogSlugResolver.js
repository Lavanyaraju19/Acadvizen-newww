...
export function findLocalBlogBySlug(slug = '', availableBlogs = localBlogs) {
  const resolvedSlug = resolveBlogSlug(slug, availableBlogs)
  if (!resolvedSlug) return null
  return availableBlogs.find((entry) => entry.slug === resolvedSlug) || null
}
...
export function resolveBlogSlug(slug = '', availableBlogs = localBlogs) {
  ...
  const exactMatch = availableBlogs.find((entry) => entry.slug === requestedSlug)
  if (exactMatch?.slug) return exactMatch.slug

  const aliasedSlug = canonicalizeKnownBlogSlug(requestedSlug)
  if (aliasedSlug !== requestedSlug && availableBlogs.some((entry) => entry.slug === aliasedSlug)) {
    return aliasedSlug
  }
