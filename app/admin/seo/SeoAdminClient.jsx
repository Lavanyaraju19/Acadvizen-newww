...
export default function SeoAdminClient() {
  ...
  async function saveSeo(event) {
    ...
    if (!form.page_slug.trim()) {
      setStatus('Page URL is required.')
      return
    }
    ...
    try {
      ...
      setStatus(error?.message || 'Failed to save SEO settings.')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function deleteSeo(id) {
    ...
    try {
      ...
      await loadSeo()
      setStatus('SEO configuration deleted.')
    } catch (error) {
      setStatus(error?.message || 'Failed to delete SEO configuration.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Surface className="p-6 md:p-8">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-semibold text-slate-50">SEO Settings</h2>
          <p className="mt-1 text-sm text-slate-300">Manage search engine optimization settings for each page.</p>
        </div>
        <button
          type="button"
          onClick={resetForm}
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[280px_1fr]">
        <aside className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          <h3 className="text-sm font-semibold text-slate-100">Page Configurations</h3>
          <div className="mt-3 space-y-2">
            {items.map((item) => (
              ...
              >
                ...
                <div className="text-xs opacity-80">{item.meta_title || 'No title set'}</div>
              </button>
            ))}
            {!items.length ? <p className="text-xs text-slate-500">No SEO configurations yet.</p> : null}
          </div>
        </aside>
