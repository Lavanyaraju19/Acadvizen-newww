...
-- Insert default hero - `on conflict do nothing` alone is not a real duplicate guard here since
-- id is a random UUID with no natural unique constraint; use `where not exists` instead so this
-- never inserts a second row alongside real content that already exists in this table.
...
-- Table may have pre-existed without these columns - see homepage_testimonials.rating above for
-- why this is added defensively rather than assumed. app/api/cms/homepage/course-modules/route.js
-- genuinely reads/writes duration, focus and pillars.
alter table public.homepage_course_modules add column if not exists duration text;
alter table public.homepage_course_modules add column if not exists focus text;
alter table public.homepage_course_modules add column if not exists pillars jsonb not null default '[]'::jsonb;
...
-- app/api/cms/homepage/projects/route.js genuinely reads/writes `link`.
alter table public.homepage_projects add column if not exists link text;
...
-- app/api/cms/homepage/placements/route.js genuinely reads/writes logo_url, student_name, testimonial.
alter table public.homepage_placements add column if not exists logo_url text;
alter table public.homepage_placements add column if not exists student_name text;
alter table public.homepage_placements add column if not exists testimonial text;
...
-- The table above may have pre-existed (created outside this migration history) without the
-- newer `rating` column - add it defensively before indexing it, same pattern used by the later
-- schema-contract-parity migrations for other tables that drifted from this file's assumptions.
alter table public.homepage_testimonials add column if not exists rating int default 5;
do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'homepage_testimonials_rating_check'
  ) then
    alter table public.homepage_testimonials
      add constraint homepage_testimonials_rating_check check (rating >= 1 and rating <= 5);
  end if;
end $$;
...
-- Table may have pre-existed without `category` (app/api/cms/homepage/faq/route.js doesn't use it,
-- but the index below assumes it - add it defensively so the index creation doesn't fail).
alter table public.homepage_faq add column if not exists category text;
...
-- app/api/cms/homepage/tools/route.js genuinely reads/writes logo_url, category, link.
alter table public.homepage_tools add column if not exists logo_url text;
alter table public.homepage_tools add column if not exists category text;
alter table public.homepage_tools add column if not exists link text;
...
-- app/api/cms/homepage/cta/route.js genuinely reads/writes title, description, background_color,
-- text_color (its own comment notes it maps to "actual DB column names from base migration" -
-- true of this file's original intent, but the table it hit in production had already drifted
-- to heading/subheading/background_image instead - add the columns the live route needs).
alter table public.homepage_cta add column if not exists title text;
alter table public.homepage_cta add column if not exists description text;
alter table public.homepage_cta add column if not exists background_color text default '#050b12';
alter table public.homepage_cta add column if not exists text_color text default '#ffffff';
...
-- Insert default CTA - same `where not exists` reasoning as the hero insert above.
