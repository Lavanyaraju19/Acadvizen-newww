...
-- Page templates table. May already exist from 202603130001_cms_expansion_pass2.sql, which
-- creates it with only id/name/slug/description/page_type/sections_json/style_json/is_active -
-- the CREATE TABLE below is then a no-op, so template_type/template_data/is_default/created_by
-- (needed by this file's own INSERT) must be added explicitly via ALTER rather than assumed
-- present.
...
insert into public.page_templates (name, slug, description, template_type, template_data, is_default) values
