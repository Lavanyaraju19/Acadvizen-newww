-- Recovery migration for 20260722_page_templates.sql (tracked, not edited here).
--
-- That file's seed INSERT for the 3 default page templates never provides a `slug`, but
-- 202607220011_page_templates.sql (the earlier-running, corrected creator of this table)
-- defines slug as NOT NULL UNIQUE - so the insert fails its not-null constraint and the 3
-- sample rows never get seeded. Low-stakes (sample content only, no app code depends on
-- these specific rows existing), but re-seeded here with slugs so the migration set leaves
-- no unresolved failure. Sorts after every other 2026-07-22 migration via the zzzz prefix.
