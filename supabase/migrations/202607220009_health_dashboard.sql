...
-- public.redirects may already exist from 202603130001_cms_expansion_pass2.sql, which uses the
-- newer from_path/to_path/status_code convention rather than this file's old_url/new_url/
-- redirect_type/hits/created_by convention - lib/redirects.js already detects and supports both
-- schemas at query time, so add the legacy columns defensively instead of assuming they exist.
alter table public.redirects
  add column if not exists old_url text,
  add column if not exists new_url text,
  add column if not exists redirect_type int default 301,
  add column if not exists hits int default 0,
